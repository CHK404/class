 ...
 * -파스칼 표기법 사용
 * -언더스코어(_) X
 * -클래스 1개 = 파일 1개 '관례적' 원칙(권장)
 * ㄴ 여러 클래스 넣으면 어떤 파일에 어떤 클래스가 들어 있는지 헷갈림
 * -충돌 위험
 * -재사용성 떨어짐
 * -버전관리(Git) 힘듬
 */
...
namespace WindowsFormsApp_07_class
{
    ...
    internal class Car
    {
         ...
         * 외부에서 직접 접근 시 문제가 생길 수 있으므로 보통 private 사용
         * 필드 이름은 camelCase로 작성
         */
        ...
        //2-2. readonly 필드
        //-readonly 키워드 사용
        //-초기화 한 이후 값을 바꿀 수 없는 필드(=불변 데이터용)
        //ㄴ생성자에서는 값을 한 번 정할 수 있음
        //Why?
        //-한 번 설정되면 절대 바뀌지 않아야 하는 값(데이터)을 보호하고 싶을 때 사용
         ...
         * ㄴ 필드에 직접 접근을 막고, 값을 읽거나 쓸 때 조건 검사 가능
         */
        ...
        /*
         * 생성자 = 객체가 만들어 질 때 (new 키워드 사용자) 자동으로 실행되는 특별한 메서드
         ...
         * ㄴ 객체가 더 이상 사용되지 않으면 -> CRL이 가비지 컬렉션으로 메모리 정리를 할 때 소멸자가 자동 호출
         * - 주로 파일 받기, DB 연결 종료 등 리소스 정리 용도
         * 
         * C#은 가비지 컬렉터가 메모리 관리하므로 자주 사용되지 안흠
        ...
        public void PrintInfo()
        {
            ...
            Console.WriteLine($"신차 여부: {(IsNewCar ? "신차" : "구형차")}");
